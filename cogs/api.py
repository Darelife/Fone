...
class Api(commands.Cog):
    ...
    async def movie(self, ctx, *, movie):
      ...
      #jsi - tmdb similar movies
        ...
        listm = discord.Embed(title = f"Movie Search Query : {movie.capitalize()}", description = f"<@!{ctx.author.id}> Please type the number preceding the movie you want to know about", color = color, timestamp = ctx.message.created_at)
        number = 1
        title_number = 1
        for x in data1["d"]:
            listm.add_field(name = f'{title_number}) {x["l"]}', value = f"""id : {x['id'][2:]} • rank : {x['rank']}
""", inline = False)
            title_number += 1
            number += 1
            if number == 11:
        ...
        try:
          ...
          with open("testmov.txt", "w") as f:
              f.write(str(data1))
          parent1 = data1.find("body").find('div', attrs={'class':'credit_summary_item'})
        ...
        def genre(n):
            n = int(n)
            if n == 28: return "Action"
            if n == 12: return "Adventure"
            if n == 16: return "Animation"
            if n == 35: return "Comedy"
            if n == 80: return "Crime"
            if n == 99: return "Documentary"
            if n == 18: return "Drama"
            if n == 10751: return "Family"
            if n == 14: return "Fantasy"
            if n == 36: return "History"
            if n == 27: return "Horror"
            if n == 10402: return "Music"
            if n == 9648: return "Mystery"
            if n == 10749: return "Romance"
            if n == 878: return "Science Fiction"
            if n == 10770: return "TV Movie"
            if n == 53: return "Thriller"
            if n == 10750: return "War"
            if n == 37: return "Western"
        try: 
          genres = []
        ...
        try:
          mov = discord.Embed(title = f'({a+1}) {jd["l"]}', description = overview, timestamp = ctx.message.created_at, color = newcolor)
        except: 
          mov = discord.Embed(title = f'({a}) {jd["l"]}', description = "Couldn't find the movie overview", timestamp = ctx.message.created_at, color = newcolor)
        mov.set_author(icon_url = "https://ancpr.org/wp-content/uploads/2019/12/imdb.png", name = "IMDB", url = imdblink)
        try:
...
#https://api.themoviedb.org/3/movie/353081
            ...
            video_id = jv["key"]
            mov.add_field(name = "Video", value = f"[`Youtube Link`](https://www.youtube.com/watch?v={video_id})", inline = False)
        except: pass
        try:
        ...
        try:
          mov.set_footer(text = f'Requested by {ctx.author}, Powered By TMDB and IMDB', icon_url = ctx.author.avatar_url)
        except: pass
        await mess.edit(embed = mov)
        #you'll need to change the emojis
...
#my recommendation embed
        ...
        recc = discord.Embed(title = f"Recommendation", description = f"More Movies Like {jd['l']}", color = random.choice(colour))
        try:
        ...
        try:
          recc.add_field(name = "Homepage Link", value = f"[Homepage Link]({homepage})", inline = True)
        except: pass
        try:
...
#similar movies embed
        ...
        try:
          s = discord.Embed(title = s_title, description = s_overview, color = random.choice(colour))
        except: pass
        try:
          s.set_author(name = "Similar Movies", icon_url = "https://i.pinimg.com/favicons/7951fa82068a0eb2399e656e8ae8770d7dce814f72d5c20c5cb224e5.png?e86714ddd80dcd75c0fce447cd2bc6a1", url = f"https://www.themoviedb.org/movie/{s_id}")
        except: pass
        try:
...
#credits embed
        ...
        with urlopen(f"https://api.themoviedb.org/3/movie/{tmbd_id}/{creditslink}") as f:
        ...
        while True:
          ...
          try:
            ...
            if reaction.emoji.id == 843714124189794325:
              await mess.remove_reaction("<:SimilarMovies:843714124189794325>", ctx.author)
              await mess.edit(embed=s)
            elif reaction.emoji.id == 843714124374605894:
              await mess.remove_reaction("<:Recommendations:843714124374605894>", ctx.author)
              await mess.edit(embed=recc)
            elif reaction.emoji.id == 843078309352570920:
              await mess.remove_reaction("<:Movie:843078309352570920>", ctx.author)
              await mess.edit(embed=mov)
            elif reaction.emoji.id == 843714124600967178:
              await mess.remove_reaction("<:Credits:843714124600967178>", ctx.author)
              await mess.edit(embed = credits)
          except asyncio.TimeoutError:
            await mess.remove_reaction("<:SimilarMovies:843714124189794325>", self.client.user)
            await mess.remove_reaction("<:Credits:843714124600967178>", self.client.user)
            await mess.remove_reaction("<:Recommendations:843714124374605894>", self.client.user)
            await mess.remove_reaction("<:Movie:843078309352570920>", self.client.user)
            break

        

    @commands.command(aliases = ["movies", "mov"])
    #this is an alternative movie command
    async def movie2(self, ctx, *, movie):
        ...
        with urlopen(f"{apilink}{movie}") as m:
            data = json.load(m)
        #(f"{apilink}{movie}")
        embed = discord.Embed(title = f"Movie Search Query : {movie.capitalize()}", description = f"<@!{ctx.author.id}> Please type the number preceding the movie you want to know about", color = color, timestamp = ctx.message.created_at)
#https://image.tmdb.org/t/p/w500/ -> for the image url
        number = 1
        titlenumber = 1
        for x in data["results"]:
          try:
            embed.add_field(name = f'{titlenumber}) {x["title"]}', value = f"""{x['id']} • {x['vote_average']}
{x['release_date'][:-6]}""", inline = False)
          except:
            embed.add_field(name = f'{titlenumber}) {x["title"]}', value = f"""{x['id']} • {x['vote_average']}
Date Of Release Not Found""", inline = False)
          number += 1
          titlenumber += 1
          if number == 11:
                break
        ...
        newcolor = random.choice(colour)
        jd = data["results"][a]
        embed2 = discord.Embed(title = jd["title"], description = jd["overview"], timestamp = ctx.message.created_at, color = newcolor)
        try:
        ...
        try:
          ...
          pass
        try:
          embed2.add_field(name = "Original Language", value = jd["original_language"], inline = False)
        ...
        try:
          embed2.add_field(name = "Rating", value = f"{jd['vote_average']} from {jd['vote_count']} votes")
        except:
          pass
        ...
        #   pass
        try:
          embed2.set_image(url = f"https://image.tmdb.org/t/p/w500{jd['backdrop_path']}")
        except:
          pass
        try:
          embed2.set_thumbnail(url = f"https://image.tmdb.org/t/p/w500{jd['poster_path']}")
        except:
          pass
        try:
    ...
    async def age(self, ctx, name:str):
        #it's just a fun command...don't blame me on anything....it's the api
        #attempts to predict your age based on the popularity of your name
        with urlopen(f"https://api.agify.io?name={name}") as cu:
            data = json.load(cu)
        age = data['age']
        embed = discord.Embed(title = "Age Guess", description = f"""<@!{ctx.author.id}> :
        your age should probably be around {age}""", timestamp = ctx.message.created_at, color = random.choice(colour))
        embed.set_footer(text = f'Requested by {ctx.author}', icon_url = ctx.author.avatar_url)
    ...
    async def bored(self, ctx):
        ...
        typ = data['type']
        embed = discord.Embed(title = "Since you are bored", description = f"""<@!{ctx.author.id}> :
        {activity}""", timestamp = ctx.message.created_at, color = random.choice(colour))
    ...
    async def bitcoin(self, ctx, cur):
        ...
        value = cur['value']
        typ = cur['type']
        embed = discord.Embed(title = "Value Of Bitcoin", description = name, timestamp = ctx.message.created_at, color = random.choice(colour))
    ...
    async def dog(self, ctx):
        ...
        with urlopen("https://dog.ceo/api/breeds/image/random") as cu:
            data = json.load(cu)
        image = data['message']
        embed = discord.Embed(title = "Here are some dog pics", description = f"<@!{ctx.author.id}>", timestamp = ctx.message.created_at, color = random.choice(colour))
    ...
    async def ip(self, ctx, ip):
      try:
        ...
        embed = discord.Embed(title = "Info Abt You through your IP address", description = f"""<@!{ctx.author.id}> :
        {ip}""", timestamp = ctx.message.created_at, color = random.choice(colour))
        ...
        await ctx.channel.send(f'Please enter a valid IP address <@!{ctx.author.id}>')

    @commands.command(pass_context=True) 
    async def meme(self, ctx): 
        #will fetch memes from r/memes only
    ...
    async def memes(self, ctx, subreddit): 
        #the user will need to specify the subreddit name without the "r/"
        embed = discord.Embed(title="", description="")
        async with aiohttp.ClientSession() as cs:
    ...
    # @commands.command()
    # async def movie(self, ctx, movie):
    #     await ctx.send("My dev. is still working on it")

    @commands.command()
    async def news(self, ctx, *,topic = None):
      api_key = "your_api_key"
      if topic == None:
        with urlopen(f"https://newsapi.org/v2/top-headlines?sources=google-news-in&apiKey={api_key}") as cu:
            data = json.load(cu)
        ...
        content = a['content']
        embed=discord.Embed(title = title, description = description, timestamp = ctx.message.created_at, url = url, color = random.choice(colour))
        embed.add_field(name = 'Content', value = content)
        embed.set_image(url = url_image)
        embed.set_footer(text = f'Requested by {ctx.author} | story by {author}', icon_url = ctx.author.avatar_url)
        await ctx.channel.send(embed=embed)
      elif topic != None:
        topic = topic.replace(' ', '+')
        print(topic)
        with urlopen(f"https://newsapi.org/v2/everything?q={topic}&from=2021-03-24&sortBy=popularity?language=en&apiKey={api_key}") as cu:
            data = json.load(cu)
    ...
    @commands.command()
    #i was a noob when i was making this command....so like i didn't know how a lot of stuff worked and i checked on some tutorials (for this command)
        ...
        if x["cod"] != "404":
            try:
                async with channel.typing():
                    ...
                    dateStr = datetime.fromtimestamp(int(woah)).strftime("%A, %B %d, %Y %I:%M:%S")
                    weather_description = str(z[0]["description"])
                    #weather_description = z[0]["description"]
                    if str(weather_description) == 'clear sky':
                        #replace the values with your own pics (they are the thumbnails of the embeds
                    ...
                    embed.add_field(name="Feels Like", value=f"**{current_feelslike_celsiuis}°C**", inline=False)
                    embed.add_field(name="Humidity(%)", value=f"**{current_humidity}%**", inline=False)
                    embed.add_field(name="Visibility", value=f"**{current_visibility}**", inline=False)
                    embed.add_field(name="Atmospheric Pressure(hPa)", value=f"**{current_pressure}hPa**", inline=False)
                    embed.add_field(name="Sunrise", value=f"**{dateStr}**", inline=False)
